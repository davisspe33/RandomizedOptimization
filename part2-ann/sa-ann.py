...
def plotaccuracy(x,y):
    ...
    for x in range(1,1001,50):
        clf = mlrose.NeuralNetwork(hidden_nodes = [], activation = 'relu', \
                                    algorithm = 'simulated_annealing', max_iters = x, \
                                    bias = True, is_classifier = True, learning_rate = 0.0001, \
    ...
    axes[0].plot(param_range, accuracy, label="simulated_annealing",color="blue", lw=2)
    ...
    axes[1].plot(param_range, times, label="simulated_annealing",color="blue", lw=2)
